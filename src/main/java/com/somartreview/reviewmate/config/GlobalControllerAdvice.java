...
public class GlobalControllerAdvice {
    ...
    public ResponseEntity<ExceptionResponse> handleRuntimeException(RuntimeException e, ContentCachingRequestWrapper requestWrapper) {
        ...
        String message = getUndefinedErrorMessage(e, requestWrapper);
        log.error(message);

        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(ExceptionResponse.builder()
                        .code(RUNTIME_ERROR.getCode())
                        .message(RUNTIME_ERROR.toString() + " : " + RUNTIME_ERROR.getMessage() + ". " + message)
                        .build());
    }

    private String getUndefinedErrorMessage(RuntimeException e, ContentCachingRequestWrapper requestWrapper) {
        return e.getMessage()
                + " <- "
